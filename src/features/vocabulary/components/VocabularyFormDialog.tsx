...
export function VocabularyFormDialog({
  ...
  const executeSearch = async (targetWord: string, isManual = false) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setSearchSuccessMessage(
          isAi
            ? `✨ [AI 정밀 검수 완료] 정확한 단어 정보가 자동 입력되었습니다.`
            : d.source
            ? `✨ [${d.source}] 정보가 자동 입력되었습니다.`
            : '✨ 단어 정보가 자동 입력되었습니다.'
        );
      } else if (!res.success) {
  ...
  // ⚡ 단어 입력 중 350ms 디바운스 실시간 자동 검색 (생성/수정 모드 모두 지원)
  useEffect(() => {
    if (!open) return;
    const clean = word.trim();
    if (clean.length < 2) return;
    if (clean.toLowerCase() === lastSearchedWord.current.toLowerCase()) return;
